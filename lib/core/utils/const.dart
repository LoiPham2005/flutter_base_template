...
class Const {
  ...
  static void checkLogin(BuildContext context, {required Function nextPage}) {
    // Lấy StorageService từ DI container
    ...
    if (storageService.isLoggedIn()) {
      ...
      // ví dụ: Get.to(() => LoginPage());
    }
  }

  static String formatTimeString(time, {String? format}) {
    if (time == null) {
      return '';
    }
  ...
  static String formatTimeStringWithoutHour(time, {String? format}) {
    if (time == null) {
      return '';
    }
    final DateTime dateTime = DateTime.parse(time);
  ...
  static String convertTime(DateTime time, {String? format}) {
    return DateFormat(
      format ?? 'dd/MM/yyyy - HH:mm:ss',
      'en_US',
    ).format(time.add(const Duration(hours: 7)));
  }
  ...
  static String formatTime(time, {String? format, bool isSecond = true}) {
    if (time == null) {
      return '';
    }
    if (isSecond) {
  ...
  static String formatTimeWithoutHour(
    time, {
    String? format,
    bool isSecond = true,
  }) {
  ...
  static String convertPrice(dynamic price) {
    final res = isNumeric(price.toString());
    if (res) {
  ...
  static double? convertNumberNull(dynamic data) {
    final res = isNumeric(data.toString());
    if (res) {
  ...
  static String convertContact(String? value) {
    if (value != null) {
      final String data = value.replaceAll(' ', '');
      final String data1 = data.replaceAll('-', '');
      final String data2 = data1.replaceAll('+', '');
  ...
  static bool checkImage(String link) {
    bool check = false;
  ...
  static Future<void> callLaunch(url) async {
    if (!await launch(url)) {
...
List<String> timeFilter(String time) {
  ...
  switch (time) {
    ...
    case 'Tháng trước':
      ...
      break;
    case '30 ngày':
      dayStart = now.subtract(const Duration(days: 30));
      dayEnd = DateTime(now.year, now.month, now.day, 23, 59, 59);
      break;
    case 'Năm nay':
  ...
  final int sTimestamp = (dayStart.millisecondsSinceEpoch / 1000).round();
  final int eTimestamp = (dayEnd.millisecondsSinceEpoch / 1000).round();
